...
router.post('/', authMiddleware, async (req, res) => {
    ...
    const { title, date, time_start, time_end, color } = req.body;
    const userId = req.user.id;
...
router.patch('/:id/incomplete', authMiddleware, async (req, res) => {
    const taskId = req.params.id;
    const userId = req.user.id;
    ...
    try {
        const { data, error } = await supabase
            .from('tasks')
            .update({ is_completed: false, updated_at: new Date() })
            .eq('id', taskId)
            .eq('user_id', userId)
            .select()
            .single();
        ...
        console.error("Error incompleting task:", err.message);
        res.status(500).send('Server Error');
    }
});

// به‌روزرسانی تسک (مثلاً توضیحات)
router.patch('/:id', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (Object.keys(updates).length <= 1) return res.status(400).json({ error: 'هیچ فیلدی برای به‌روزرسانی ارسال نشده.' });
